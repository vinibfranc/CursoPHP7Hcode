...
class User extends Model {
    ...
    {
        base64_decode($code);
        $idrecovery = mcrypt_decrypt(MCRYPT_RIJNDAEL_128, base64_decode($code),MCRYPT_MODE_ECB);
        $sql = new Sql();
        $results = $sql->select("
        SELECT * FROM
    ...
    public static function setForgotUsed($idrecovery){
        $sql = new Sql();
        $sql->query("UPDATE tb_userspassawordsrecoveries SET dtrecovery = NOW() idrecovery = :idrecovery", array(
            ":idrecovery"=>$idrecovery
        ));
    }
